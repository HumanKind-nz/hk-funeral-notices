...
class HK_Funeral_GitHub_Updater {
	...
	private const CACHE_KEY = 'hk_funeral_notices_github_response';
	...
	private function get_repository_info() {
		...
		if (is_wp_error($response)) {
			error_log("HK Funeral Notices GitHub API request failed: " . $response->get_error_message());
		...
		if ($response_code !== 200) {
			error_log("HK Funeral Notices GitHub API request failed with response code: " . $response_code);
			// Cache error response to prevent constant retries
			set_transient(self::CACHE_KEY, ['status' => 'error'], self::ERROR_CACHE_DURATION * HOUR_IN_SECONDS);
			return false;
		}
		...
		if (!isset($body->tag_name, $body->assets) || empty($body->assets)) {
			error_log("HK Funeral Notices GitHub API response missing required fields or assets.");
			// Cache error response to prevent constant retries
			set_transient(self::CACHE_KEY, ['status' => 'error'], self::ERROR_CACHE_DURATION * HOUR_IN_SECONDS);
			return false;
		}
		...
		if (empty($body->zipball_url)) {
			error_log("HK Funeral Notices: No valid download URL found for the latest release.");
			// Cache error response to prevent constant retries
			set_transient(self::CACHE_KEY, ['status' => 'error'], self::ERROR_CACHE_DURATION * HOUR_IN_SECONDS);
			return false;
		}
	...
	public function check_update($transient) {
		...
		if (defined('WP_DEBUG') && WP_DEBUG) {
			error_log("HK Funeral Notices GitHub Updater - Current version: {$current_version_normalized}, Latest version: {$latest_version}");
		}
